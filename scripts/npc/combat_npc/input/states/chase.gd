...
func generate_input() -> InputPackage:
	...
	for character in character_folder.get_children():
		if character.is_in_group("player"):
			...
			handler.target = handler.char_controller.global_position.lerp(character.position, 
				1-handler.target_distance/handler.char_controller.global_position.distance_to(character.position))
